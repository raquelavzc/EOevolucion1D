...
!=========================================================
! ELIMINAR SNAPSHOTS DE SIMULACIONES ANTERIORES
!=========================================================

call execute_command_line('cmd /c del /q snapshot_*.txt 2>nul', &
     wait=.true., exitstat=exitstat)

if (exitstat == 0) then
   print*
   print*, 'Snapshots anteriores eliminados.'
else
   print*
   print*, 'No se encontraron snapshots anteriores o no se pudieron eliminar.'
end if
...
write(10,'(A13,1X,A23)') 'r', 'phi'

do j = 0, Nr-1

   write(10,'(ES24.16E3,1X,ES24.16E3)') r(j), phi_now(j)
...
do n = 1, Nt
   ...
   if (mod(n,20) == 0) then
      ...
      do j = 0, Nr-1
         ...
         write(10,'(ES24.16E3,1X,ES24.16E3)') r(j), phi_new(j)
